    ...
    var body: some View {
        NavigationStack {
            Group {
                switch selectedTab {
                case .today:
                    TodayView(meals: meals) { mealBeingEdited = $0 }
                case .scan:
...
private enum AppTab {
    case today, scan, log
    var title: String { switch self { case .today: "Today"; case .scan: "Scan"; case .log: "Log" } }
    var icon: String { switch self { case .today: "sun.max.fill"; case .scan: "viewfinder"; case .log: "book" } }
}

private enum MealType: String, CaseIterable, Identifiable, Codable {
    case breakfast = "Breakfast", lunch = "Lunch", dinner = "Dinner", snack = "Snack"
    var id: String { rawValue }
}

private struct Meal: Identifiable, Codable, Equatable {
    let id: UUID
    var name: String
    var detail: String
    ...
    init(id: UUID = UUID(), name: String, detail: String, mealType: MealType, quantity: Double, calories: Int, protein: Int, carbs: Int, fat: Int, date: Date) {
        self.id = id; self.name = name; self.detail = detail; self.mealType = mealType
        self.quantity = quantity; self.calories = calories; self.protein = protein
        self.carbs = carbs; self.fat = fat; self.date = date
    }
...
private struct DetectedFood: Identifiable {
    let id = UUID()
    let name: String
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 18) {
                ...
                CalorieSummary(consumed: calories, goal: goal)
                HStack {
...
private enum AppTheme { static let accent = Color(red: 0.04, green: 0.55, blue: 0.36) }
